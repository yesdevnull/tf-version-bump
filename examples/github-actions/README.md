...
Each of the secret's values is registered with `::add-mask::`, which redacts it from the workflow console only. A multi-line value is registered as a single mask, so the console may not redact it line by line. When an update, `terraform init`, `fmt` or `validate` command fails, its output is printed in the `Process candidate` step log, where those masks apply. Step logs are kept for the repository's log retention period, 90 days by default, rather than the artefact's seven, so a multi-line credential a provider echoes can stay visible there for longer. Every command's output is also captured to log files inside the processing artefact, and those files are not redacted: the artefact is retained for seven days and can be downloaded by anyone with read access to the repository, so a credential a provider echoes into Terraform's output appears there in plaintext. Supplied variables at least cannot turn on Terraform's trace logging, because `TF_LOG` and `TF_LOG_PATH` are reserved.
...
The callers pin `tf-version-bump` to `v1.0.0-rc.17` and verify archive SHA-256 `137eac8239d08e2a93409e9eda1fd59b52abcf3d774fa7c3882ce53534f56a4d` before running it.
...
A same-repository pull request into the default branch that changes a caller's configuration file or the caller workflow itself starts that caller's `preview` job. It discovers the policy's branches exactly as a live run does, then processes one branch per configured prefix, with its update, `terraform init`, optional formatting and `terraform validate`, and publishes nothing: the `publish` job is skipped, and publication also refuses any ref but the default branch. Pull requests into another branch skip the preview. The preview job skips pull requests from forks. A Dependabot pull request that changes a watched file, such as an action pin in a caller, does run the preview, because Dependabot's branches live in this repository; GitHub gives Dependabot runs Dependabot secrets rather than the repository's Actions secrets, so such a preview has no `TF_API_TOKEN` or `TERRAFORM_ENV` unless you also store them as Dependabot secrets.

The preview job shares the live job's inputs through YAML aliases, so a pull request that edits a pin, a prefix or a root is previewed with its edits. When a preview runs, it uses the pull request's own scripts and reusable workflow, including any edits to them; editing those files alone does not start a preview. A newer push to the pull request cancels its older preview; previews never wait behind or block a live run.
...
GitHub runs pull-request workflows only for the `opened`, `synchronize` and `reopened` activities, and not at all while the pull request has a merge conflict. Re-running the workflow picks up moved state branches, because discovery lists them live, but a GitHub re-run reuses the original event's `GITHUB_SHA`, so reflecting a moved default branch needs a new pull-request event instead, such as a push (for example updating the branch from the default branch) or closing and reopening the pull request.
...
- **Version report** (`version-report.csv`): one row per check with `status`, `branch`, `kind`, `subject`, `block`, `file`, `actual`, `expected` and `detail` columns. It checks each root's presence, whether `main.tf` and `providers.tf` exist, `required_version`, providers and modules. A value that already matches passes, which includes a later entry whose row shows the version an earlier entry for the same source would set, so a branch is current only when none of its rows fail; a module the config's `ignore_modules`, `ignore_versions` or `from` excludes is `SKIP`, naming the filter, as is a module with a local source; a branch that cannot be fetched, parsed or safely read — a symlinked Terraform file, a root outside the checkout, or a duplicate root — is `ERROR`. The summary counts every status and lists each branch's non-passing rows.
...
Both CSVs keep values as the audit reports them: as written, except that when a config has several module entries for one source, a later entry's row shows the version an earlier entry would set, as [the version audit](../../docs/USAGE.md#machine-readable-version-audit) describes. A missing version is rendered rather than passed through: as above, the legacy report writes `none`, and the version report leaves the cell empty. A value beginning with `=`, `+`, `-` or `@`, such as the valid Terraform pin `= 5.0.0`, may be evaluated as a formula by a spreadsheet that opens the file directly, so import the CSVs as text instead.
...
Open the workflow run to inspect `discover`, `process` and `publish`. Each `process` job's summary includes the updater's log for every configured root, so the run overview page shows which versions changed; a long log is truncated there and kept in full in the artefact. In a preview, the summary also shows each changed branch's candidate patch, truncated the same way. Terraform's own logs stay out of the summary, because a provider can echo a credential into them; when a command fails, the `Process candidate` step log shows its output, masked as described in [Supply environment variables](#supply-environment-variables). Each summary links that job's result artefact, whose name identifies the branch only by a hash. In a dry run, each `publish` job's summary states what a live run would have done to the branch's pull request and failure issue. Download its result artefacts to see `result.json`, `candidate.patch` and captured command logs. Artefacts are retained for seven days. Results belong to one run attempt; use **Re-run all jobs**, as partial job reruns are unsupported.
