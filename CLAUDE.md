...
Every summary ends with `failureNote`, which reports `N update(s) failed; see the errors above` whenever the mode's error count is non-zero, so a summary of what succeeded is never mistaken for the whole run. A run that updated nothing and failed omits its success line rather than counting no files as a success, and config mode says nothing about the config in that case — a failed run must not send the operator to the config when the files or their permissions are at fault. The two config-mode no-op messages are distinct: `Config.declaresNoUpdates` (the single predicate `-validate-config` also uses) names a config that asks for nothing, and everything else is reported as already applied or matching nothing, pointing at `-audit-file` for which. Note the two readers deliberately draw different conclusions from that predicate: `-validate-config` rejects such a config, while an update run reports it and exits 0.
