...
module "vpc-prod" {
  ...
  version = "3.0.0"
	...
	updated, err := updateModuleVersion(tmpFile, "terraform-aws-modules/vpc/aws", "5.0.0", "", ignorePatterns, false, false, false)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func TestFilePermissionPreservation(t *testing.T) {
  ...
  version = "3.0.0"
	...
	// Update the file
	_, err = updateModuleVersion(tmpFile, "terraform-aws-modules/vpc/aws", "5.0.0", "", nil, false, false, false)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func TestMultipleFilesSimultaneously(t *testing.T) {
  ...
  version = "3.0.0"
	...
	for _, file := range files {
		updated, err := updateModuleVersion(file, "terraform-aws-modules/vpc/aws", "5.0.0", "", nil, false, false, false)
		if err != nil {
