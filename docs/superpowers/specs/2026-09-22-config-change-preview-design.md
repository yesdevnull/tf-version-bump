...
The design depends on two GitHub Actions behaviours the documentation does not settle. Both are verified in a disposable private repository before any implementation work, with a probe that mirrors the caller shape below. The probe's structure has already passed the pinned actionlint 1.7.12 and `yq` alias resolution locally.

1. **Job-level `queue: max`.** GitHub's documentation describes `queue` only for workflow-level `concurrency`. The design moves each caller's live concurrency to the job level. If job-level `queue: max` is rejected or ignored, the fallback keeps workflow-level concurrency with an event-aware group, `github.repository_id` plus a `-pr-<number>` suffix on pull-request events, and `queue: max`. Previews then queue per pull request instead of cancelling, and the README says so.
2. **A read-only preview job calling a workflow whose skipped `publish` job requests write permissions.** GitHub documents that a called workflow can only downgrade the caller's `GITHUB_TOKEN` permissions. It does not say whether a job the `if` skips is still checked. If the preview job cannot declare `contents: read`, the fallback gives it the caller's existing permissions, and publication relies on the `publish` job's `if` together with the publication guard.
...
- **Two jobs.** Both call the reusable workflow, with the same `with:` and `secrets:` blocks. The live job defines them as anchors (`with: &policy-inputs`, `secrets: &policy-secrets`) and the preview job uses the aliases (`with: *policy-inputs`, `secrets: *policy-secrets`). Every policy value is therefore written once, and a pull request that edits a value, such as a tool pin, a prefix or a root, is previewed with it.
  - `automation`, the live job: `if: github.ref == refs/heads/<default branch> && github.event_name != 'pull_request'`, with job-level `concurrency: {group: tf-version-bump-<policy>-<repository id>, cancel-in-progress: false, queue: max}`. Its behaviour is otherwise unchanged.
  - `preview`: `if: github.event_name == 'pull_request' && github.event.pull_request.head.repo.full_name == github.repository && github.event.pull_request.base.ref == github.event.repository.default_branch`, with job-level `concurrency: {group: tf-version-bump-<policy>-preview-<repository id>-<pull request number>, cancel-in-progress: true}` and `permissions: contents: read` (subject to prerequisite 2).
...
- **Shared rendering.** The existing fence-escalation and 64 KiB truncation code becomes one shell function in the step, `append_fenced_file <heading> <file>`, used for each updater log and for the patch. The patch holds only `.tf` changes, formatting changes and root lock files (`write_candidate`, `process-state-branch.sh`), never Terraform's logs, so it cannot carry a credential Terraform echoed.
