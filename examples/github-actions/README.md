...
A same-repository pull request into the default branch that changes a caller's configuration file or the caller workflow itself starts that caller's `preview` job. It discovers the policy's branches exactly as a live run does, then processes one branch per configured prefix, with its update, `terraform init`, optional formatting and `terraform validate`, and publishes nothing: the `publish` job is skipped, and publication also refuses any ref but the default branch. Pull requests into another branch skip the preview. The preview job also skips pull requests from forks, although GitHub may instead fail such a run, or a Dependabot pull request's, at startup, because their token is read-only.
