...
func TestUpdateModuleVersionWithCount_CountsSkipsThatLeaveBlocksUnpinned(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantWarning: "is not a registry module and cannot use a version attribute, skipping",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantWarning != "" && !strings.Contains(warnings, tt.wantWarning) {
				t.Fatalf("warnings = %q, want one containing %q", warnings, tt.wantWarning)
			}
		})
	}
}
