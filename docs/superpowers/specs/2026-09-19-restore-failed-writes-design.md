...
Known limitation: On Linux, writeback errors are reported once and then cleared by the kernel, so after a rewrite's `Sync` fails, a subsequent `Sync` during the restore can report success even though the restored bytes were not persisted to storage. The tool then removes the backup and reports a failure with a message ending `original content restored`, so the user is assured their content is safe when it may not be. This limitation is inherent to the `gofmt` model chosen, but it is still preferable to the previous behaviour where the same failure left a truncated file with no backup copy.
