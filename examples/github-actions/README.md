...
Both CSVs keep values as the audit reports them: as written, except that when a config has several module entries for one source, a later entry's row shows the version an earlier entry would set, as [the version audit](../../docs/USAGE.md#machine-readable-version-audit) describes. The legacy report is the one exception: as above, it writes `none` where the audit has no value. A value beginning with `=`, `+`, `-` or `@`, such as the valid Terraform pin `= 5.0.0`, may be evaluated as a formula by a spreadsheet that opens the file directly, so import the CSVs as text instead.
